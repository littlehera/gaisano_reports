...
def execute(filters=None):
	...
	department = filters.get("department") if filters.get("department") is not None else ""
	section = filters.get("section") if filters.get("section") is not None else ""
	category = filters.get("category") if filters.get("category") is not None else ""

	supplier = filters.get("supplier")

	if report_type == "Past 90 Days":
		data = get_data_3months(from_date, to_date, branch, business_unit, supplier, division, department, section, category)
	else:
		data = get_data_total(from_date, to_date, branch, business_unit, supplier, division, department, section, category)
...
def get_columns(report_type, from_date, to_date):
	...
	if report_type == "Total Only":
		columns = [
			...
			{"label": "Total Offtake (Case)", "fieldname": "total_offtake_case", "fieldtype": "Float", "precision":2, "width": 120},
			{"label": "Ave. Daily Offtake (Case)", "fieldname": "ave_daily_offtake_case", "fieldtype": "Float", "precision":2, "width": 120}
		]
		...
		columns = [
			...
			{"label": "Total Offtake (Case)", "fieldname": "total_offtake_case", "fieldtype": "Float", "precision":2, "width": 120},
			{"label": "Ave. Daily Offtake (Case)", "fieldname": "ave_daily_offtake_case", "fieldtype": "Float", "precision":2, "width": 120},
			{"label": "Ave. Monthly Offtake (Case)", "fieldname": "ave_monthly_offtake_case", "fieldtype": "Float", "precision":2, "width": 120}
		]
	return columns

def get_data_total(from_date, to_date, branch, business_unit, supplier, division,  department, section, category):
	data = []
	where_clause = ""
	conditions = []
	...
	if department != "":
		conditions.append("p.department_id = %s"%department)

	if section != "":
		conditions.append("p.section_id = %s"%section)

	if category != "":
		conditions.append("p.category_id = %s"%category)
	...
	for row in rows:
		...
		packing = float(row[5] if row[5] is not None else 1)
		data.append({
			"item_name": row[0],
			"barcode": row[1],
			"uom": row[2],
			...
			"total_offtake_case": float(total_offtake)/packing,
			"ave_daily_offtake": float(total_offtake) / float((to_date - from_date).days),
			"ave_daily_offtake_case": float(total_offtake)/packing / float((to_date - from_date).days),
			"ave_monthly_offtake": float(total_offtake) / float((to_date - from_date).days) * 30,
			"ave_monthly_offtake_case": 30 * float(total_offtake)/packing / float((to_date - from_date).days)
		})

	return data

def get_data_3months(from_date, to_date, branch, business_unit, supplier, division, department, section, category):
	...
	total_data = get_data_total(from_date, to_date, branch, business_unit, supplier, division,  department, section, category)
	m1_data = get_data_total(m1_from, m1_to, branch, business_unit, supplier, division, department, section, category)
	m2_data = get_data_total(m2_from, m2_to, branch, business_unit, supplier, division, department, section, category)
	m3_data = get_data_total(m3_from, m3_to, branch, business_unit, supplier, division, department, section, category)
	...
	for row in total_data:
		data.append({
			...
			"total_offtake_case": row["total_offtake_case"],
			"ave_daily_offtake": row["total_offtake"]/90,
			"ave_daily_offtake_case": row["total_offtake_case"]/90,
